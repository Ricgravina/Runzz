...
import { ArrowLeft, Plus, X, Search, CheckCircle, Pill } from 'lucide-react';
...
export default function Medications() {
    ...
    useEffect(() => {
        const p = getProfile();
        setProfile(p || { gender: 'male', weight: 70, height: 175, medications: [], customChips: [] });
    }, []);
    ...
    const handleToggle = (item: string) => {
        ...
        const updated = { ...profile, medications: next };
        setProfile(updated);
        saveProfile(updated);
    };
    ...
    const handleAddCustom = () => {
        ...
        const updated = {
            ...profile,
            medications: [...(profile.medications || []), searchTerm],
            customChips: [...custom, { category: 'medication' as const, label: searchTerm }]
        };
        ...
        <div className="p-6 min-h-screen bg-background text-text font-sans">
            <header className="flex items-center gap-4 mb-8">
                ...
                <div className="flex-1">
                    <h1 className="text-2xl font-black">Medications</h1>
                    <p className="text-xs text-text/60 font-medium">Affecting gut motility or absorption</p>
                </div>
            </header>
            ...
            <div className="space-y-6">
                <div>
                    <div className="space-y-3">
                        {commonMeds.map(med => {
                            const isSelected = profile.medications?.includes(med);
                            return (
                                <button
                                    key={med}
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className={`font-bold ${isSelected ? 'text-secondary' : 'text-text-inverse'}`}>{med}</span>
                                    </div>
                ...
                <div className="mt-8 pt-8 border-t border-black/5">
                    <h3 className="font-bold text-lg mb-4">Other Medications</h3>

                    <div className="flex flex-wrap gap-2 mb-4">
                        {profile.customChips?.filter(c => c.category === 'medication').map(chip => (
                            <button
                                key={chip.label}
                                onClick={() => handleToggle(chip.label)}
                                className={`px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 border ${profile.medications?.includes(chip.label) ? 'bg-primary/10 border-primary text-secondary' : 'bg-surface border-black/5 text-secondary'}`}
                            >
                                {chip.label}
                                {profile.medications?.includes(chip.label) && <X size={14} className="opacity-50 hover:opacity-100" onClick={(e) => { e.stopPropagation(); handleToggle(chip.label); }} />}
                            </button>
                        ))}
                    </div>
